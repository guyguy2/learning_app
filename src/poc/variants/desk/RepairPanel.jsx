...
/**
 * RepairPanel
 * A dedicated repair card (never a toast) for diagnosing mental model errors.
 * Renders the misconception name, pedagogical explanation, notional machine tiles
 * with the mistaken form struck through, and a single retest button.
 */
export default function RepairPanel({ repair, lastAttempt, onRetry, family }) {
    ...
    <div className="desk-card desk-card--slide desk-repair-panel">
      ...
      <div className="desk-btn-group">
        <button
          ...
          autoFocus
        >
          {isNamedMisconception ? 'Try a similar one' : 'Try again'}
        </button>
      </div>
    </div>
  )
