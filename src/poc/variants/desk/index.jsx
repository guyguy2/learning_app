...
/**
 * DeskVariant
 * Design POC Variant with Skola-style physicality:
 * Warm paper index cards on a desk, tactile serif display typography,
 * morphological paper slips, and commit-then-reveal feedback.
 */
function DeskVariant() {
  ...
  // Wrap attempt submission with feedback & retest alignment tracking
  async function handleAttemptSubmit(attemptPayload) {
    ...
    if (attemptPayload.correct) {
      ...
      if (retestActive) {
        ...
        setTimeout(() => setMentalModelAligned(false), 3500)
      }
    } else {
      setLocalFeedback(null)
    }
  ...
  return (
    <div className="variant-desk" style={{ '--card-family-color': familyColor }}>
      {/* Top Meta Bar */}
      <div className="desk-topbar">
        <div className="desk-session-tag">
          <span>Desk Edition</span>
          {mode === 'review' && (
      ...
      {/* 4. Active Misconception Repair Panel */}
      {repair && (
        <RepairPanel
          repair={repair}
...
export default {
  id: 'desk',
  name: 'Desk',
  description: 'Warm paper index cards on a desk; tactile, serif display type.',
  Component: DeskVariant,
}
