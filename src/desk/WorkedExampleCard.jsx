 ...
 */
export default function WorkedExampleCard({ stimulus, onAcknowledge, technique }) {
  ...
  // Enter key advances steps when not focused on textarea
  useState(() => {
    // initial state
  })
  ...
  return (
    <div className="desk-card desk-card--slide" onKeyDown={handleKeyDown}>
      ...
      {!isCompleted ? (
        <div className="desk-btn-group">
          <button
            ...
            autoFocus
          >
            Next step ({step + 1} of {paradigm.length}) <span className="desk-keycap">Enter</span>
          </button>
        </div>
      ) : (
